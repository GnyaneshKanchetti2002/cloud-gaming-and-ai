...
export default function LoginPage() {
  ...
  useEffect(() => {
    // 1. CATCH THE TOKEN: After login, Render redirects back here with ?token=...
    const token = searchParams.get('token');

    if (token) {
      // 2. SAVE THE TOKEN: Store it so the Landing Page knows we are logged in
      localStorage.setItem('token', token);
      
      // 3. REDIRECT: Send the user to the gaming dashboard
      router.push('/gaming');
    }
  }, [searchParams, router]);
    ...
    <div className="flex flex-col md:flex-row h-screen w-full bg-black overflow-hidden font-sans">
        ...
        <div className="w-full md:w-1/2 bg-[#0b1120] border-b md:border-b-0 md:border-r border-slate-800 flex flex-col justify-center items-center p-12 relative group">
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              {/* Updated href to use dynamic baseUrl */}
