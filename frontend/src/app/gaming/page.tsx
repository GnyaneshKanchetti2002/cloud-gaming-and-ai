...
export default function GamingDashboard() {
  ...
  const fetchInstances = async (userId: number) => {
    try {
      const res = await fetch(`${API_BASE_URL}/proxmox/instances/${userId}`, { headers: getAuthHeaders() });
      if (res.ok) setInstances(await res.json());
    } catch (e) { console.error(e); }
  };

  const fetchWallet = async (userId: number, resolution: string) => {
    try {
      const res = await fetch(`${API_BASE_URL}/users/wallet/${userId}`, { headers: getAuthHeaders() });
      if (res.ok) {
        const data = await res.json();
        // Route correct balance based on user configuration
        let hrs = data.esports_hours || 0;
        if (resolution === "1440p") hrs = data.aaa_hours || 0;
        if (resolution === "4K") hrs = data.ultra_hours || 0;
        
        setWalletSeconds(Math.floor(hrs * 3600));
  ...
  useEffect(() => {
    const authenticate = async () => {
      ...
      try {
        ...
        const resConfig = activeUser.target_resolution || "1080p";
        setTargetResolution(resConfig);
        
        fetchInstances(activeUser.id);
        fetchWallet(activeUser.id, resConfig);
  ...
  const handleIdleShutdown = async () => {
    if (activeGame) {
      try {
        await fetch(`${API_BASE_URL}/proxmox/idle-shutdown/${activeGame.id}`, { method: "POST", headers: getAuthHeaders() });
    ...
    try {
      await fetch(`${API_BASE_URL}/proxmox/provision`, {
        ...
        body: JSON.stringify({
          ...
          vram_allocation: targetResolution === "4K" ? 24 : targetResolution === "1440p" ? 12 : 8,
  ...
  const handleKill = async (instanceId: number) => {
    try {
      await fetch(`${API_BASE_URL}/proxmox/kill/${instanceId}`, { method: "DELETE", headers: getAuthHeaders() });
      if (user?.id) fetchInstances(user.id);
    } catch (e) { console.error(e); }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 pb-24 px-4 sm:px-6 lg:px-8 relative animate-in fade-in duration-700">
      ...
      {showIdleWarning && (
        <IdleOverlay onAction={() => setShowIdleWarning(false)} onShutdown={handleIdleShutdown} />
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-center mt-12">
        ...
        <div className="xl:col-span-1">
          <NodeHealthCard resolution={targetResolution} />
        </div>
      </div>
